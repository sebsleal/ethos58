...
const Layout = ({ children }) => {
  ...
  return (
    <div className="h-screen bg-gray-50 dark:bg-[#09090B] text-gray-900 dark:text-gray-100 flex flex-col md:flex-row overflow-hidden font-sans selection:bg-brand-500/20 selection:text-brand-700">
      ...
      {/* Mobile Header */}
      <div className="md:hidden flex items-center justify-between p-6 z-20 border-b border-gray-200 dark:border-zinc-800">
      ...
      {/* Sidebar Navigation */}
      <nav className={cn(
        "flex-shrink-0 transition-all duration-300 ease-in-out z-10 flex flex-col border-r border-gray-200 dark:border-zinc-800",
        isMobileMenuOpen
          ? "absolute inset-x-0 top-[80px] bottom-0 w-full bg-gray-50 dark:bg-[#09090B] px-6"
          : "hidden md:flex md:h-screen w-[260px] p-6 lg:p-8 bg-white dark:bg-[#09090B]"
      )}>
        {/* Sidebar Header / Logo */}
        <div className="mb-10 mt-2 px-2 hidden md:block">
      ...
      {/* Main Content Area */}
      <main className="flex-1 flex flex-col overflow-hidden relative bg-gray-50 dark:bg-[#09090B]">
        <div className={cn("absolute top-6 right-8 z-10 hidden md:flex", location.pathname === '/viewer' && "!hidden")}>
        ...
        <div className={cn(
          ...
          location.pathname === '/viewer' ? "p-0" : "px-4 md:px-8 lg:px-12 pt-8 md:pt-16 pb-12"
        )}>
          <div className={cn(
